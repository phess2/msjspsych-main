...
#############################
# IPad display functionality
#############################

async def index(request):
    return aiohttp.web.FileResponse("array_display_interface.html")

async def reset_speaker_grid(websocket, list_name_disabled):
    # Turn on all the speaker icons
    ...
    # Disable speakers with negative elevations 
    await websocket.send(
        json.dumps(
            {
                "disabled": list_name_disabled,
                "message": "",
            }
        )
    )

async def recolor_speaker_grid(websocket, list_name_disabled):
    # Turn on all the speaker icons
    await websocket.send(
        json.dumps(
            {
    ...
    await websocket.send(
        json.dumps(
            {
                "recolor": list_name_disabled,
                "message": "Press key to start next trial",
            }
        )
    )
...
async def zap_speakers(
    speaker_config: dict, duration: int, DB_SPL_SPEAKER_WAKE: int = 35,
) -> None:
...
async def setup_trial(
    ...
    if idx % block_length == 0 or (resume and idx == resume_idx):
        ...
        await zap_speakers(speaker_config, duration=5, DB_SPL_SPEAKER_WAKE=DB_SPL_SPEAKER_WAKE)
    ...
    if block_length is not None and idx % block_length == 0:
        ...
        if make_output_file and responses:
...
async def run_exp(idx, trial, speaker_config, break_sound, break_soundsr, block_start_sound, block_start_soundsr, block_length=50, dBA=50, DB_SPL_SPEAKER_WAKE=30):
    await speaker_utils.force_unroute_all(speaker_config)
    await setup_trial(idx, speaker_config, break_sound, break_soundsr, block_start_sound, block_start_soundsr, 
                      make_output_file=False, responses=None, DB_SPL_SPEAKER_WAKE=DB_SPL_SPEAKER_WAKE, block_length=block_length)
    await run_trial(trial, speaker_config, 0.5, dBA)
